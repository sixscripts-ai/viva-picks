...
async def init_db():
    ...
    # Wallet table (Added user_id column)
    # Note: For existing Main Wallet legacy support, user_id can be 'main_wallet' or NULL initially
    # On Turso this is a new table, so we can just create it right.
    await db_manager.execute_write("""
...
def get_password_hash(password):
    # Bcrypt (used by passlib) has a 72 byte limit
